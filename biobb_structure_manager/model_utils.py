...
def check_unk_at_in_r(res, at_list, skip_H=True):
    ...
    if skip_H:
        return  [at.id for at in res.get_atoms() if at.element != 'H' and at.id not in at_list['backbone'] + at_list['side'] + ['OXT']]
    else:
        return  [at.id for at in res.get_atoms() if at.id not in at_list['backbone'] + at_list['side'] + ['OXT']]
...
def get_ligands(struc, incl_water=False):
    ...
    """
    return [res for res in struc.get_residues() if is_hetatm(res) and (incl_water or not is_wat(res))]
